    ...
    function format(d) {
        ...
        if (d.estado === 'Activo' || d.estado === 'Atrasado' || d.estado === 'Fecha de Vencimiento cercano') {
            ...
            if (d.usuario_ejecutor === usuarioActual) {
                acciones += '<button class="accion-btn" title="Finalizar Tarea" id="' + d.id_relacion + '" name="finalizar_tarea" onclick="botones(this.id, this.name, \'tareas\', "' + d.tabla_relacion + '", "' + d.tipo + '")"><i class="fas fa-check"></i></button>';
            }
        }
